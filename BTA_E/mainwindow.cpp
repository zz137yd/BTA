...
{
    // Get the root directory path of the current left and right views
    QString leftRootPath = fileModel->filePath(leftView->rootIndex());
    QString rightRootPath = fileModel->filePath(rightView->rootIndex());

    // Clear the delegate cache (only clear the current directory and its subdirectories)
    if (sizeDelegate)
    {
        ...
        {
            if (key.startsWith(leftRootPath)) cache.remove(key);
        }
        // Clean up pendingPaths to avoid duplicate tasks
        ...
        {
            if (key.startsWith(leftRootPath)) pending.remove(key);
        }
    }

    // Force model refresh
    fileModel->setRootPath(leftRootPath);
...
void MainWindow::upClicked()
{
    ...
    if (fPath == initialPath || flag)
    {
        fPath = "";
        mainPath = initialPath;
        fLabel->setText(QString("Favorites list: %1").arg(fCategory));
        fLabel->show();
...
void MainWindow::favoritesDialog(const QString& category)
{
    ...
    dialog->setWindowTitle(QString("Collections - %1").arg(fCategory));
    ...
    // Progress bar
    fSizeDelegate = new SizeProgressDelegate(fileModel, this);
    ...
    fSearchModel = new QStandardItemModel(this);
...
void MainWindow::clearCutStateIfNeeded()
{
    ...
    // No longer determine whether the current index is a cut file, clear it directly
    clipPaths.clear();
    clipIsCut = false;
    leftView->viewport()->update();
    rightView->viewport()->update();
...
void MainWindow::openWith(const QString& application)
{
    ...
    QString filePath = fileModel->filePath(idx);
    ...
    QProcess::startDetached(appToRun, QStringList() << filePath);
}
...
void MainWindow::Compressions(const QString &format)
{
    ...
    if (!idx.isValid())
    {
        QMessageBox::warning(this, "Error", "Please select a file or folder before compressing.");
        return;
    }

    QString srcPath = fileModel->filePath(idx);
    if (srcPath.isEmpty())
    {
        QMessageBox::warning(this, "Error", "Unable to get the path of the selected item.");
        return;
    }
    ...
    if (format == "7z")
    {
        ...
        {
            QMessageBox::warning(this, "Missing environment", "7z environment not detected.");
            return;
        }
    ...
    {
        QMessageBox::warning(this, "Unsupported format", QString("Unsupported format: %1").arg(format));
        return;
    }
    ...
    m_compressProc = new QProcess(this);

    // 7z/zip/tar branch (using 7z) with real progress
    if (!usePowerShell && !useTar)
    {
        ...
        connect(m_compressProc, &QProcess::readyReadStandardError, this, [this, updateProgress]() {
            updateProgress(m_compressProc->readAllStandardError());
        });
        connect(m_compressProc, &QProcess::readyReadStandardOutput, this, [this, updateProgress]() {
    ...
    connect(m_compressProc,
        ...
        this, [=](int exitCode, QProcess::ExitStatus status) {
            ...
            if (exitCode == 0)
            {
                QMessageBox::information(this, "Finished", QString("Compressed file： \n%1").arg(dstPath));
                renameCompressions(dstPath);
            }
            else
                QMessageBox::warning(this, "Failed", QString("Compression failed (Exit Code %1)").arg(exitCode));
...
void MainWindow::collect(const QString& category, const QString& path)
{
    if (!favorites[category].contains(path))
    {
        ...
        saveFavoritesToFile();
        QMessageBox::information(this, "Add to favorites", QString("Successfully added path \"%1\" to the “%2” collection.").arg(path, category));
    }
    else
        QMessageBox::information(this, "Add to favorites", QString("Path \"%1\" already exists in “%2” collection.").arg(path, category));
}
...
void MainWindow::createNew(const QString &type)
{
    ...
    QModelIndex currIdx = leftView->rootIndex();
    ...
    // Pop up input box to get the user's file name
    QInputDialog dlg(this);
    ...
    // Create synchronously/asynchronously based on type
    bool syncSuccess = false;

    // Directory: Synchronous creation
    if (type == "directory") syncSuccess = QDir(dirPath).mkdir(finalName);
    ...
    else if (!useCom)
    {
        ...
        else if (type == "image")
        {
            ...
            syncSuccess = img.save(fullPath, "PNG");
        }
        else if (type == "txt")
        {
            QFile file(fullPath);
            syncSuccess = file.open(QIODevice::WriteOnly | QIODevice::Text);
            file.close();
        }
    }
    ...
    if (useCom)
    {
        // Copy to local variables for lambda to use
        QString localFullPath = fullPath;
        QString localType     = type;

        // Performing COM operations on background threads
        QFuture<bool> future = QtConcurrent::run([=]() -> bool
        {
            ...
            if (localType == "docx")
            {
                ...
                doc->dynamicCall("SaveAs(const QString&)",
                                 QDir::toNativeSeparators(localFullPath));
                doc->dynamicCall("Close()");
                word.dynamicCall("Quit()");
                ok = true;
            }
            else if (localType == "xlsx")
            {
            ...
            return ok;
#else
            Q_UNUSED(localFullPath);
            Q_UNUSED(localType);
            return false;
#endif
        });

        // Use watcher to return to the main thread to refresh
        // or pop up a warning after the background execution is completed
        ...
        connect(watcher, &QFutureWatcher<bool>::finished,
            ...
            if (!ok)
            {
                QMessageBox::warning(this, "Error",
                                     QString("Unable to create %1 file: \n%2")
                                     .arg(type).arg(localFullPath));
            }
            else
            {
                qDebug() << "Create Successfully: " << localFullPath;
                refreshView();
            }
        });
    ...
    {
        // Non-COM synchronous branch, immediate feedback
        if (!syncSuccess)
        {
            QMessageBox::warning(this, "Error",
...
void MainWindow::copyFilePath()
{
    ...
    QToolTip::showText(QCursor::pos(), "Copied file path", this);
}
...
void MainWindow::createShortCut()
{
    ...
    {
        QMessageBox::warning(this, "Failed", "No files or folders selected");
        return;
    }

    QString targetPath = fileModel->filePath(currentContextIndex);
    QFileInfo fi(targetPath);
    QString name = fi.fileName() + tr(" - shortcut");
    QString dir = fileModel->filePath(leftView->rootIndex());
    QString linkPath;

#ifdef Q_OS_WIN
    linkPath = QDir(dir).filePath(name + ".lnk");
    if (!createWindowsShortcut(targetPath, linkPath))
        QMessageBox::warning(this, "Failed", "Unable to create shortcut");
#elif defined(Q_OS_LINUX)
    linkPath = QDir(dir).filePath(name + ".desktop");
    if (!createLinuxDesktopFile(targetPath, linkPath))
        QMessageBox::warning(this, "Failed", "Unable to create desktop shortcut");
#elif defined(Q_OS_MAC)
    linkPath = QDir(dir).filePath(name + ".alias");
    if (!createMacAlias(targetPath, linkPath))
        QMessageBox::warning(this, "Failed", "Unable to create alias");
#endif
...
void MainWindow::paste(const QString& dir)
{
    ...
    if (!fileConflicts.isEmpty())
    {
        ...
        auto btn = QMessageBox::question(this, "File overwrite confirmation",
    ...
    if (!dirConflicts.isEmpty())
    {
        ...
        auto btn = QMessageBox::question(this, "Folder overwrite confirmation",
        ...
        else
        {
            if (fi.isDir())
            {
                ...
                while (it.hasNext()) {
                    QFileInfo sfi(it.next());
                    emit currentFile(sfi.fileName());
                    if (sfi.isFile()) deleted += sfi.size();
...
void MainWindow::toRecyleBin()
{
    ...
    if (totalSize <= 1 * 1024 * 1024 * 1024)
    {
        ...
        if (!allOk)
            QMessageBox::warning(this, tr("Operation failed"), tr("Some options cannot be moved to the Trash"));
        else
            updateRightView();
        return;
    }
    ...
    connect(worker, &FileDeleteTask::finished, this, [=](bool success, const QString &err) {
        if (dialog) dialog->close();
        if (!success) QMessageBox::warning(this, "Operation failed", err);
        else updateRightView();
        thread->quit();
        thread->wait();
        worker->deleteLater();
